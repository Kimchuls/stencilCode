...
def countErrorBound(exp):
    if exp > 0:
        return "1E+{0}".format(exp)
    else:
        return "1E-{0}".format(abs(exp))
    ...
    for x in range(sx, nx + 1):
        for y in range(sy, ny + 1):
            for z in range(sz, nz + 1):
                for compressor in range(1, 3):
                    for SZerrorBoundMode in range(2):
                        for etype in range(1, -11, -1):
                            fileopen = open(filepath, 'w')
                            fileopen.write("x = {0}\ny = {1}\nz = {2}\n".format(x, y, z))
                            fileopen.write("iteration = {0}\nsteps = {1}\n".format(iters, steps))
                            fileopen.write("compressor = {0}\n".format(compressor))
                            fileopen.write("SZerrorBoundMode = {0}\n".format("ABS" if SZerrorBoundMode else "REL"))
                            fileopen.write("SZabsErrorBound = {0}\n".format(countErrorBound(etype)))
                            ...
                            log = "x = {0}, y = {1}, z = {2}, ".format(x, y, z) \
                                  ...
                                  + "SZerrorBoundMode_{0}, ".format("ABS" if SZerrorBoundMode else "REL") \
...
if __name__ == '__main__':
    ...
    if args.type == 0:
        ...
        changeVariable(3, 4, 4, 3, args.parameter_2, args.parameter_3, args.folder)
    else:
        changeVariable(4, 4, 4, args.parameter_1, args.parameter_2, args.parameter_3, args.folder)
