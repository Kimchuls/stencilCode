...
def train_LINEAR(fileList, noList, inputPath, outputPath):
    dataSet = []
    ...
    for iter, item in enumerate(fileList):
        ...
        for iter1 in range(injectnum):
            list1 = []
            start = int(int(lines[2 + (outputLeng + 1) * iter1].replace("******* Injected Iters: ", "").replace(" *******\n", ""))/10)
    ...
    np.save(os.path.join(outputPath, "npSet_LINEAR.npy"), npSet)
    print(npSet.shape)


def test_LINEAR(fileList, noList, inputPath, outputPath):
    dataSet = []
    tap = os.path.join(inputPath, 'loadfile.txt')
    tapFile = open(tap, "r")
    tapLines = tapFile.readlines()
    anslist = open(os.path.join(outputPath, "anslist_LINEAR.txt"), "w")
    for iter, item in enumerate(fileList):
        if (iter + 1) not in noList:
            continue
        print(iter, item)
        ...
        for iter1 in range(injectnum):
            ...
            if testmode == 1:
                break
    npSet = np.array(dataSet)
    np.save(os.path.join(outputPath, "npSet_LINEAR-1.npy"), npSet)
    anslist.close()
    print(npSet.shape)
...
if __name__ == '__main__':
    ...
    train_LINEAR(trainItems, args.trainputno, args.traininput, args.trainoutput)
    test_LINEAR(testItems, args.testinputno, args.testinput, args.testoutput)
