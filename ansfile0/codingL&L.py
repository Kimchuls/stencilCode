...
def decode_LSTM(fileList, anslistpath, npSetpath):
    ...
    for iter, item in enumerate(fileList):
        ...
        for iter1 in range(0, 150, 15):
            ...
            for i in range(iter1, 150 - length - 1):
                item_dataSet = list1[i:i + length ]
                dataSet.append(item_dataSet)
                for anslist_item in list1[i + length +1]:
...
if __name__ == '__main__':
    items = getFileName()
    decode_LSTM(items, "./file/anslist_LSTM_{0}-1.txt".format(17), "file/npSet_LSTM_{0}.npy".format(17))
    # for i in range(16):
    #     x=set()
    #     while len(x)<4:
